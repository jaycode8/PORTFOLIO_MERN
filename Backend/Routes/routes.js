...
router.get("/", async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Successfully deployed REST API" });
  } catch (error) {
    res.json({ success: false, message: "Encounter fatal error" });
  }
});
router.post("/test", upload.single("projectPhoto"), (req, res) => {
  const imgUrl = req.file.path;
  const publicId = req.file.filename;
  res.json({ imgUrl: imgUrl, publicId: publicId });
});
router.delete("/test", (req, res) => {
  const publicId = "Projects/gzjwpgfklbnk3qdavh8k";
  const del = cloudinary.uploader.destroy(publicId);
  res.json({ msg: "successfully deleted image from cloudinary server" });
});
...
router.post("/addLanguage", async (req, res) => {
  try {
    if (
      ...
      const alreadyExist = await proLangModel.find({
        language: req.body.language,
      });
      if (alreadyExist == "") {
        const existColor = await proLangModel.find({
          langColor: req.body.langColor,
        });
        if (existColor == "") {
          ...
          if (savedLanguage) {
            res.json({
              success: "true",
              message: `Successfully added ${req.body.language} to the database`,
            });
          }
        } else {
          res.json({
            success: "false",
            message: `The color (${req.body.langColor}) is alredy assigned to ${existColor[0].language}`,
          });
        }
      } else {
        res.json({
          success: "false",
          message: "Programming Language already exists in the database",
        });
      }
    }
  } catch (error) {
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const year = req.body.year;
      const file = req.file;
      const bgImgs = req.file.filename;
      technology = technology.split(",").map((ele) => ele.trim());
      const bgImg = {
      ...
      if (
        ...
        if (file) {
          ...
          if (existProjectTitle == "") {
            const existProjectLInk = await projectsModel.find({
              linkto: linkto,
            });
            if (existProjectLInk == "") {
              ...
              if (savedProject) {
                ...
                res.json({
                  success: "false",
                  message: "error during project saving to database",
                });
              }
            } else {
              res.json({
...
router.delete("/project/:id", async (req, res) => {
  try {
    ...
    const publicId = searchProject[0].bgImg.publicId;
    const delProject = await projectsModel.deleteOne({ _id: id });
    if (!delProject) {
      res.json({
        success: "false",
        message: "Error.. project was not deleted",
      });
    } else {
  ...
  try {
    if (
      ...
      if (savedSchool) {
        res.json({
          success: "true",
          message: "successfully added the Institution to db",
        });
      } else {
...
router.put("/institution/:id", upload.single("schoolImg"), async (req, res) => {
  try {
    ...
    if (
      ...
      const updateInstitution = await schoolModel.findByIdAndUpdate(
        req.params.id,
        schoolData
      );
...
router.delete("/institution/:id", async (req, res) => {
  try {
    ...
    if (deleteSchool) {
      ...
      res.json({
        success: "true",
        message: "sucessfully deleted the institution",
      });
    } else {
    ...
    if (req.body.service == "" || req.body.description == "") {
      ...
      if (existService == "") {
        ...
        if (savedService) {
          res.json({
            success: "true",
            message: "Successfully saved service in db",
          });
        } else {
...
router.get("/service/:id", async (req, res) => {
  try {
    ...
    res.json({
      success: false,
      message: " Encoutered error while loading services",
    });
  }
});
